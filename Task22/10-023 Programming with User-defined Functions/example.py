...
# It's just a name we give it so we know how to refer to it within the function 
# (every indented line under the def add_one(x) line is 'within' the function).
# The code indented under 'def add_one' is the logic of the function. It defines 
# what happens when the function is called. Simply put, the function computes a 
# new variable y, which is the value stored in variable x with 1 added to it.
...
# would like to pass into it within parentheses.
...
# Think of the 'call to the function', i.e. add_one(num) , as a 'placeholder' 
# for some computation. The function will go off and run its code, and return 
# its result in that place.


# ========= Function Parameters ===========
# In the function definition, you put the names of variables that you want to store 
# the input values to, between the parentheses after the function name.
# You can put more than one of these variables or parameters, simply separate them by commas.

# ************ Example 4 ************
# Here we've created a function which accepts two arguments and returns the result 
# of the base to the power exponent using the ** operation (where x ** y means x^y).
print("\nExample 4: ")
...
do_something()
print(j) # The output here is still 5. This is because, within the do_something()
# function, j = 7 made a *new* local variable j - it didn't change the outer j. To
# get the function to print out 7, we'd need to place a print statement *inside*
# the function.
...
#   parameters -- num1, num2, num3. Then, write logic to create a new variable, y, 
#   that is the sum of all three of these numbers. Then, return the result y.
#   Now, after you've defined this function, write a function call to return the 
#   sum of the numbers 52, 25, and 1403.
